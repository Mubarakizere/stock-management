...
class RawMaterialController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $categories = Category::active()->forRawMaterials()->orderBy('name')->get();
        ...
        return view('raw-materials.index', compact('products', 'categories', 'threshold'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'category_id' => [
                ...
                Rule::exists('categories','id')->where(fn($q) =>
                    $q->whereNull('deleted_at')->where('is_active', true)->where('kind', 'raw_material')
                ),
            ],
        ...
        try {
            ...
            DB::commit();
            Log::info('Raw material created', ['product_id' => $product->id]);

            return redirect()->route('raw-materials.index')->with('success', 'Raw material created successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Raw material create failed', ['error' => $e->getMessage()]);
            return back()->withErrors(['error' => 'Failed to create raw material: ' . $e->getMessage()])->withInput();
        }
    }

    /** Show */
    public function show(Product $raw_material)
    {
        ...
        $purchaseHistory = \App\Models\PurchaseItem::where('product_id', $product->id)
            ->with(['purchase' => fn($q) => $q->with('supplier')])
            ->latest('id')
            ->take(20)
            ->get();

        return view('raw-materials.show', compact('product', 'totalIn', 'totalOut', 'current', 'purchaseHistory'));
    }
    ...
    public function edit(Product $raw_material)
    {
        $product    = $raw_material;
        $categories = Category::active()->forRawMaterials()->orderBy('name')->get();
        return view('raw-materials.edit', compact('product', 'categories'));
    }
    ...
    public function update(Request $request, Product $raw_material)
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('Raw material updated', ['product_id' => $product->id]);

            return redirect()->route('raw-materials.index')->with('success', 'Raw material updated successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Raw material update failed', ['error' => $e->getMessage()]);
            return back()->withErrors(['error' => 'Failed to update raw material: ' . $e->getMessage()])->withInput();
        }
    }
    ...
    public function destroy(Product $raw_material)
    {
        try {
            $raw_material->delete();
            Log::info('Raw material deleted', ['product_id' => $raw_material->id]);
            return redirect()->route('raw-materials.index')->with('success', 'Raw material deleted successfully.');
        } catch (\Throwable $e) {
            Log::error('Raw material delete failed', ['error' => $e->getMessage()]);
            return back()->withErrors(['error' => 'Failed to delete raw material: ' . $e->getMessage()]);
        }
    }
}
