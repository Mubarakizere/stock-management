            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" class="form-select" required>
                        <option value="debit" @selected(old('type', $entry->type)=='debit')>Debit (Money Out)</option>
                        <option value="credit" @selected(old('type', $entry->type)=='credit')>Credit (Money In)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ old('amount', $entry->amount) }}"
                ...
                <div>
                    ...
                    <select name="customer_id" class="form-select">
                        ...
                        @foreach($customers ?? [] as $customer)
                            <option value="{{ $customer->id }}" @selected(old('customer_id', $entry->customer_id)==$customer->id)>
                                {{ $customer->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="supplier_id" class="form-select">
                        ...
                        @foreach($suppliers ?? [] as $supplier)
                            <option value="{{ $supplier->id }}" @selected(old('supplier_id', $entry->supplier_id)==$supplier->id)>
                                {{ $supplier->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="form-textarea">{{ old('description', $entry->description) }}</textarea>
                </div>
            </div>
