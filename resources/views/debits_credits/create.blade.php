    ...
    <form action="{{ route('debits-credits.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded-lg shadow">
        ...
        <div>
            ...
            <input type="number" name="amount" step="0.01" value="{{ old('amount') }}"
                   class="w-full border rounded-lg p-2" placeholder="Enter amount" required>
        </div>
        ...
        <div>
            ...
            <textarea name="description" class="w-full border rounded-lg p-2" rows="2" placeholder="Optional note...">{{ old('description') }}</textarea>
        </div>
