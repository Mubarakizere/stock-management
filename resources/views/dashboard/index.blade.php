<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        @if(in_array($role, ['admin','manager']) && $hasData)
        <section class="space-y-4">
            <h3 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <i data-lucide="alert-triangle" class="w-4 h-4 {{ ($rawMaterialsLowStock ?? 0) > 0 ? 'text-amber-500' : 'text-gray-300' }}"></i>
                    </div>
                    <p class="text-2xl font-bold {{ ($rawMaterialsLowStock ?? 0) > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400' }}">{{ $rawMaterialsLowStock ?? 0 }}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('raw-materials.index') }}" class="text-xs text-indigo-600 hover:underline">View all</a>
                    </div>
                        ...
                        <div class="space-y-3">
                            @foreach($lowStockRawMaterials as $rm)
                                @php
                                    ...
                                    $pct     = $thresh > 0 ? min(100, ($stock / $thresh) * 100) : 0;
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('productions.index') }}" class="text-xs text-indigo-600 hover:underline">View all</a>
                    </div>
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                            ...
                                            <td class="px-3 py-2">
                                                @php
                                                    ...
                                                    $sc = $statusColors[$prod->status ?? ''] ?? 'bg-gray-100 text-gray-600';
                                                @endphp
                                                <span class="inline-flex px-1.5 py-0.5 rounded text-[0.65rem] font-medium {{ $sc }}">
                                                    {{ ucfirst($prod->status ?? '—') }}
                                                </span>
                                            </td>
                                            <td class="px-3 py-2 text-gray-500">{{ $prod->created_at->format('M d') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center py-10 text-gray-400">
                            ...
                            <a href="{{ route('productions.create') }}" class="mt-2 text-xs text-indigo-600 hover:underline">Start a production run →</a>
                        </div>
                    @endif
                </div>
